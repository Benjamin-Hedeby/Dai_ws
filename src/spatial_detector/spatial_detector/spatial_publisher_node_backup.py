...
class SpatialPublisher(Node):
    ...
    def detections_cb(self, msg: Detection3DArray):
        ...
        for det in msg.detections:
            if not det.results:
                continue
            # Extract 3D position and convert from mm to m
            orig_pos = det.results[0].pose.pose.position
            pose = Pose()
            pose.position.x = orig_pos.x / 1000.0
            pose.position.y = orig_pos.y / 1000.0
            pose.position.z = orig_pos.z / 1000.0
